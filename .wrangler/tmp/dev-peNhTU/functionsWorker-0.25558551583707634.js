...
// .wrangler/tmp/bundle-0hkSNI/checked-fetch.js
...
// .wrangler/tmp/bundle-0hkSNI/strip-cf-connecting-ip-header.js
...
// .wrangler/tmp/pages-Cte9Z7/functionsWorker-0.25558551583707634.mjs
...
__name2(verifyRecaptcha, "verifyRecaptcha");
var onRequestPost = /* @__PURE__ */ __name2(async (context) => {
  ...
  try {
    ...
    if (env.RECAPTCHA_SECRET_KEY && recaptchaToken) {
      const isValid2 = await verifyRecaptcha(recaptchaToken, env.RECAPTCHA_SECRET_KEY);
      if (!isValid2) {
        return new Response(
...
var SchoolSubmissionSchema = external_exports.object({
  schoolName: external_exports.string().min(2, "School name must be at least 2 characters").max(150).trim(),
  streetAddress: external_exports.string().max(200).trim().optional().or(external_exports.literal("")),
  city: external_exports.string().max(100).trim().optional().or(external_exports.literal("")),
  state: external_exports.enum(US_STATES).optional().or(external_exports.literal("")),
  zipCode: external_exports.string().regex(/^\d{5}(-\d{4})?$/, "ZIP must be 12345 or 12345-6789").optional().or(external_exports.literal("")),
  contactEmail: external_exports.string().email("Enter a valid email").toLowerCase().trim().optional().or(external_exports.literal("")),
  phone: external_exports.string().transform((val) => val ? val.replace(/\D/g, "") : "").refine((val) => val === "" || val.length === 10, "Phone must be 10 digits").transform((val) => val && val.length === 10 ? `(${val.slice(0, 3)}) ${val.slice(3, 6)}-${val.slice(6)}` : "").optional().or(external_exports.literal("")),
  website: external_exports.string().transform((val) => {
    if (!val) return "";
    if (!val.startsWith("http://") && !val.startsWith("https://")) return `https://${val}`;
    return val;
  }).optional().or(external_exports.literal("")),
  programs: external_exports.array(external_exports.string()).max(10).default([]),
  programOther: external_exports.string().max(200).optional().or(external_exports.literal("")),
  schoolDescription: external_exports.string().max(1e3).optional().or(external_exports.literal("")),
  submitterName: external_exports.string().max(100).optional().or(external_exports.literal("")),
  // reCAPTCHA is optional; if provided and secret exists, we verify
  recaptchaToken: external_exports.string().optional().or(external_exports.literal(""))
});
var rateLimitMap = /* @__PURE__ */ new Map();
var RATE_LIMIT_MAX = 20;
var RATE_LIMIT_WINDOW = 24 * 60 * 60 * 1e3;
...
async function verifyRecaptchaIfPresent(token, secret) {
  ...
  try {
    ...
    const data = await response.json();
    if (data.success && data.score !== void 0) return data.score >= 0.5;
    return data.success;
  } catch {
    return true;
  }
}
__name(verifyRecaptchaIfPresent, "verifyRecaptchaIfPresent");
__name2(verifyRecaptchaIfPresent, "verifyRecaptchaIfPresent");
...
// .wrangler/tmp/bundle-0hkSNI/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-0hkSNI/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.25558551583707634.js.map
